...
data:$(this).serializeArray(),success:function(e){var k=[],f="";k=$(e).contents();Array.from(k).forEach(function(h){"container"===h.parentNode.className&&(f=h)});if(/TypechoJoeTheme/.test(e)){var m=location.href;m=a.changeURLArg(m,"jscroll","comments");$.toast({type:"success",message:"\u53d1\u8868\u6210\u529f\uff0c\u5373\u5c06\u5237\u65b0\u672c\u9875\uff01"});setTimeout(function(){window.location.href=m},a.options.reloadTime)}else $.toast({type:"warning",message:f.textContent||""}),$(c).removeAttr("data-disabled")}}))})};
d.prototype.init_comment_submit=function(){var a=this;$("#comment-form").on("submit",function(b){var c=this;b.preventDefault();if(""===$("#comment-nick").val().trim())return $.toast({type:"warning",message:"\u8bf7\u8f93\u5165\u60a8\u7684\u6635\u79f0\uff01"});if(!/\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/.test($("#comment-mail").val()))return $.toast({type:"warning",message:"\u8bf7\u8f93\u5165\u6b63\u786e\u7684\u90ae\u7bb1\uff01"});b=$("#comment-content");if("canvas"===$("#commentTypeContent .canvas").attr("data-type")){var e=
$("#draw")[0].toDataURL("image/webp",.1);b.val("{!{"+e+"}!} ")}if(""===b.val().trim())return $.toast({type:"warning",message:"\u8bf7\u8f93\u5165\u8bc4\u8bba\u5185\u5bb9\uff01"});$(this).attr("data-disabled")||($(this).attr("data-disabled",!0),$.ajax({url:$(this).attr("action"),type:"post",data:$(this).serializeArray(),success:function(k){var f=[],m="";f=$(k).contents();Array.from(f).forEach(function(g){"container"===g.parentNode.className&&(m=g)});if(/TypechoJoeTheme/.test(k)){var h=location.href;
...
a(0,255)+", "+a(0,255)+", "+a(.8,1)+")")})};d.prototype.init_wap_search=function(){$(".search-toggle-xs").on("click",function(){$(".j-slide").removeClass("active");$(".j-sidebar-xs").removeClass("active");var a=$(".j-search-down-xs");a.toggleClass("active");a.hasClass("active")?($("body").css("overflow","hidden"),$(".j-header").css("box-shadow","none")):($("body").css("overflow",""),$(".j-header").css("box-shadow",""))});$(".j-search-down-xs .mask").on("click",function(){$(".j-search-down-xs").removeClass("active");
...
d.prototype.init_lazy_load=function(){new LazyLoad(".lazyload")};d.prototype.init_head_title=function(){var a=$("header.j-header"),b=$(".row.above .above-nav"),c=$(".row.below"),e=$("#post_top_title"),k=!0,f=!0,m=function(){e.addClass("post_no");b.removeClass("post_no");c.slideDown("fast",function(){k=!0})},h=function(){e.removeClass("post_no");b.addClass("post_no");c.slideUp("normal",function(){f=!0})};$(document).ready(function(){if(!(768>screen.width)){var g=a.width(),l=0;0<e.length&&750<g&&$(window).scroll(function(){var n=
$(window).scrollTop();400<n&&500>n||(450<=n?l>n&&f?(k=!1,m()):k&&(f=!1,h()):m(),l=n)})}})};d.prototype.init_navigation=function(){$("#siteNav li.nav-list-item").on("click",function(a){a.preventDefault();$(this).siblings().removeClass("active");$(this).addClass("active");window.scroll({top:$($(this).children("a").attr("href")).offset().top-($(".j-header").height()+20),behavior:"smooth"})})};"undefined"!==typeof module&&"undefined"!==typeof module.exports?module.exports=d:window.Joe=d})();
