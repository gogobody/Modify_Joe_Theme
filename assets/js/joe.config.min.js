...
function(){$("#draw").prop("width",$("#commentTypeContent").width())});$("#cancel-comment-reply-link").unbind("click").bind("click",function(){$("#draw").prop("width",$("#commentTypeContent").width())})};b.prototype.init_leaving=function(){function a(d,e){return Math.floor(Math.random()*(e-d+1))+d}var c=100;$("#j-leaving li").each(function(d,e){$(e).find(".body .content p").each(function(k,f){k=$(f).html();/\{!\{.*\}!\}/.test(k)&&(k=k.replace(/{!{/,"").replace(/}!}/,""),$(f).html('<img class="canvas" src="'+
k+'" />'))});$(e).css({"z-index":a(1,99),"background-color":"rgba("+a(0,255)+", "+a(0,255)+", "+a(0,255)+", "+a(.8,1)+")",top:parseInt(Math.random()*($("#j-leaving").height()-$(e).height()),10),left:parseInt(Math.random()*($("#j-leaving").width()-$(e).width()),10),display:"flex"});$(e).draggabilly({containment:!0});$(e).on("dragStart",function(k){c++;$(e).css("z-index",c)})})};b.prototype.init_wap_search_click=function(){$(".j-search-toggle").unbind("click").bind("click",function(){$(this).toggleClass("active");
...
!0),e.text("\u53d1\u8868\u4e2d..."),$.ajax({url:$(this).attr("action"),type:"post",data:$(this).serializeArray(),success:function(k){var f=[],m="";f=$(k).contents();Array.from(f).forEach(function(g){"container"===g.parentNode.className&&(m=g)});if(/TypechoJoeTheme/.test(k)){var h=location.href;h=a.changeURLArg(h,"jscroll","comments");$.toast({type:"success",message:"\u53d1\u8868\u6210\u529f\uff01"});setTimeout(function(){pjax.loadUrl(h)},a.options.reloadTime)}else $.toast({type:"warning",message:m.textContent||
""}),$("#j-dynamic-form-text").val(""),$(d).removeAttr("data-disabled");e.text("\u7acb\u5373\u53d1\u8868")},error:function(k){e.text("\u7acb\u5373\u53d1\u8868")}}))})};b.prototype.init_comment_submit=function(){var a=this;$("#comment-form").off("submit").on("submit",function(c){var d=this;c.preventDefault();if(""===$("#comment-nick").val().trim())return $.toast({type:"warning",message:"\u8bf7\u8f93\u5165\u60a8\u7684\u6635\u79f0\uff01"});if(!/\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/.test($("#comment-mail").val()))return $.toast({type:"warning",
message:"\u8bf7\u8f93\u5165\u6b63\u786e\u7684\u90ae\u7bb1\uff01"});var e=$("#comment-content");if("canvas"===$("#commentTypeContent .canvas").attr("data-type"))return $.toast({type:"warning",message:"\u7ba1\u7406\u5458\u7981\u6b62\u753b\u56fe~"});if(""===e.val().trim())return $.toast({type:"warning",message:"\u8bf7\u8f93\u5165\u8bc4\u8bba\u5185\u5bb9\uff01"});if(!$(this).attr("data-disabled")){$(this).attr("data-disabled",!0);$(this).find("button[type='submit']").html("\u8bf7\u7b49\u5f85...");var k=
$(".comment-btn");k.text("\u8bc4\u8bba\u4e2d..");$.ajax({url:$(this).attr("action"),type:"post",data:$(this).serializeArray(),success:function(f){var m=[],h="";m=$(f).contents();Array.from(m).forEach(function(l){"container"===l.parentNode.className&&(h=l)});if(/TypechoJoeTheme/.test(f)){var g=location.href;g=a.changeURLArg(g,"jscroll","comments");$.toast({type:"success",message:"\u53d1\u9001\u6210\u529f\uff0c\u5373\u5c06\u5237\u65b0\u672c\u9875\uff01"});setTimeout(function(){pjax.loadUrl(g)},a.options.reloadTime)}else $.toast({type:"warning",
message:h.textContent||""}),$(d).removeAttr("data-disabled"),e.val(""),$(d).find("button[type='submit']").html("\u53d1\u8868\u8bc4\u8bba");k.text("\u53d1\u8868\u8bc4\u8bba")},error:function(f){console.log($(f));$.toast({type:"warning",message:"\u53d1\u751f\u672a\u77e5\u9519\u8bef"});$(d).removeAttr("data-disabled");e.val("");k.text("\u53d1\u8868\u8bc4\u8bba")}})}})};b.prototype.init_wap_cloud=function(){var a=function(c,d){return Math.floor(Math.random()*(d-c+1))+c};$("#search-cloud a").each(function(c,
...
function(){$(".j-search-down-xs").removeClass("active");$("body").css("overflow","");$(".j-header").css("box-shadow","")})};b.prototype.init_wap_sidebar=function(){$(".j-slide").unbind("click").bind("click",function(a){$(".j-search-down-xs").removeClass("active");$("body").css("overflow","");$(".j-header").css("box-shadow","");$(this).toggleClass("active");$(".j-sidebar-xs").toggleClass("active");$(this).hasClass("active")?$("body").css("overflow","hidden"):$("body").css("overflow","")});$(".j-sidebar-xs .mask").unbind("click").bind("click",
function(){$(".j-slide").removeClass("active");$(".j-sidebar-xs").removeClass("active");$("body").css("overflow","")});$(".j-sidebar-xs .item ul li a").unbind("click").bind("click",function(a){var c=$(this);c.parent().siblings(".active").toggleClass("active");if(c.next().is("ul")){if(c.parent().toggleClass("active")&&a.preventDefault())return!1}else $(".j-slide").removeClass("active"),$(".j-sidebar-xs").removeClass("active"),$("body").css("overflow","")})};b.prototype.init_wow=function(){"on"===window.JOE_CONFIG.IS_MOBILE&&
...
(c.vod_director||"\u672a\u77e5")+"</span>\n\t\t\t\t\t\t\t</dd>\n\t\t\t\t\t\t\t<dd>\n\t\t\t\t\t\t\t\t<span>\u7b80\u4ecb\uff1a</span>\n\t\t\t\t\t\t\t\t<span>"+(c.vod_content?c.vod_content:c.vod_blurb)+"</span>\n\t\t\t\t\t\t\t</dd>\n\t\t\t\t\t\t</dl>\n\t\t\t\t\t");var e=c.vod_play_from.split("$$$"),k=c.vod_play_url.split("$$$"),f=new Map;e.forEach(function(p,q){f.set(p,k[q]||[])});e={};for(var m=$jscomp.makeIterator(f),h=m.next();!h.done;e={$jscomp$loop$prop$str$3:e.$jscomp$loop$prop$str$3},h=m.next()){var g=
$jscomp.makeIterator(h.value);h=g.next().value;g=g.next().value.split("#");e.$jscomp$loop$prop$str$3="";g.forEach(function(p){return function(q){p.$jscomp$loop$prop$str$3+='\n\t\t\t\t\t\t\t\t<li data-link="'+d(q).link+'">'+d(q).name+"</li>\n\t\t\t\t\t\t\t"}}(e));$("#j-video-play").append('\n\t\t\t\t\t\t\t<div class="video-list-play">\n\t\t\t\t\t\t\t\t<div class="title">\u6e90\uff1a'+h+'</div>\n\t\t\t\t\t\t\t\t<ul class="list-item">\n\t\t\t\t\t\t\t\t\t'+e.$jscomp$loop$prop$str$3+"\n\t\t\t\t\t\t\t\t</ul>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t")}var l=
$("#j-video-player iframe");if(l.attr("data-src")&&""!==l.attr("data-src")&&($(".video-list-play .list-item li").unbind("click").bind("click",function(){$(".video-list-play .list-item li").removeClass("active");$(this).addClass("active");l.attr("src",l.attr("data-src")+$(this).attr("data-link"));sessionStorage.setItem("playUrl",$(this).attr("data-link"))}),e=$("#j-video-play .video-list-play:first-child .list-item li:first-child"),0!==e.length)){if(sessionStorage.getItem("playUrl")){var n=null;$(".video-list-play .list-item li").each(function(p,
q){$(q).attr("data-link")===sessionStorage.getItem("playUrl")&&($(q).addClass("active"),n=!0)});!0===n?l.attr("src",l.attr("data-src")+sessionStorage.getItem("playUrl")):(l.attr("src",l.attr("data-src")+e.attr("data-link")),e.addClass("active"))}else l.attr("src",l.attr("data-src")+e.attr("data-link")),e.addClass("active");$("#j-video-player-title").html("\u6b63\u5728\u64ad\u653e\uff1a"+c.vod_name)}}}))};b.prototype.init_j_tabs=function(){$(".j-tabs .nav span").unbind("click").bind("click",function(){var a=
$(this).attr("data-panel");$(this).addClass("active").siblings().removeClass("active");$(this).parents(".j-tabs").find(".content div").hide();$(this).parents(".j-tabs").find(".content div[data-panel="+a+"]").show()})};b.prototype.init_j_collapse=function(){$(".j-collapse .collapse-head").unbind("click").bind("click",function(){var a=$(this).next();a.slideToggle(200);$(".j-collapse .collapse-body").not(a).slideUp()})};b.prototype.init_aside_motto=function(){var a=$(".j-aside-motto");"on"===window.JOE_CONFIG.DOCUMENT_ASIDE_MOTTO?
a.show():$.ajax({url:window.JOE_CONFIG.DOCUMENT_MOTTO_API,method:"get",dataType:"text",success:function(c){$(".j-aside-motto").html(c);$(".j-aside-motto").show()},error:function(c){$(".j-aside-motto").html("\u4eba\u751f\u4e4b\u8def\uff0c\u96be\u514d\u574e\u5777\uff0c\u4f46\u6211\u6267\u7740");$(".j-aside-motto").show()}})};b.prototype.init_comment_like=function(){$(".j-comment-like").unbind("click").bind("click",function(){var a=this;if($(this).hasClass("active"))return $.toast({type:"warning",message:"\u672c\u6761\u8bc4\u8bba\u60a8\u5df2\u7ecf\u8d5e\u8fc7~"});
$.ajax({url:window.location.href,type:"post",data:"likeup="+$(this).attr("data-coid"),timeout:3E4,cache:!1,success:function(c){/\d/.test(c)&&$(a).find("span").html(c.trim());$.toast({type:"success",message:"\u70b9\u8d5e\u6210\u529f\uff01"});$(a).addClass("active")}})})};b.prototype.init_dynamic_reply=function(){var a=this,c=$(".j-dynamic-reply");$(document).on("click",function(){return c.hide()});$(".j-comment-reply").unbind("click").bind("click",function(d){d.stopPropagation();$(this).parents("li").find(".j-dynamic-reply").toggle()});
c.unbind("click").bind("click",function(d){return d.stopPropagation()});c.off("submit").on("submit",function(d){var e=this;d.preventDefault();if(""===$(this).find("input[name='author']").val().trim())return $.toast({type:"warning",message:"\u8bf7\u8f93\u5165\u60a8\u7684\u6635\u79f0\uff01"});if(!/\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/.test($(this).find("input[name='mail']").val()))return $.toast({type:"warning",message:"\u8bf7\u8f93\u5165\u6b63\u786e\u7684\u90ae\u7bb1\uff01"});var k=$(this).find("textarea[name='text']");
if(""===k.val().trim())return $.toast({type:"warning",message:"\u8bf7\u8f93\u5165\u56de\u590d\u5185\u5bb9\uff01"});$(this).attr("data-disabled")||($(this).attr("data-disabled",!0),$.ajax({url:$(".j-comment-url").val(),type:"post",data:$(this).serializeArray(),success:function(f){var m=[],h="";m=$(f).contents();Array.from(m).forEach(function(l){"container"===l.parentNode.className&&(h=l)});if(/TypechoJoeTheme/.test(f)){var g=location.href;g=a.changeURLArg(g,"jscroll","comments");$.toast({type:"success",
message:"\u53d1\u9001\u6210\u529f\uff0c\u5373\u5c06\u5237\u65b0\u672c\u9875\uff01"});setTimeout(function(){pjax.loadUrl(g)},a.options.reloadTime)}else $.toast({type:"warning",message:h.textContent||""}),k.val(""),$(e).removeAttr("data-disabled")}}))})};b.prototype.init_video_album=function(){function a(c,d,e,k){d=void 0===d?1:d;e=void 0===e?1:e;k=void 0===k?.5:k;var f=document.createElement("VIDEO");f.setAttribute("src",c);f.crossOrigin="*";f.currentTime=d;return new Promise(function(m,h){f.addEventListener("loadeddata",
function(){var g=document.createElement("canvas");g.width=f.videoWidth*e;g.height=f.videoHeight*e;g.getContext("2d").drawImage(f,0,0,g.width,g.height);m(g.toDataURL("image/webp",k))})})}$(".j-short-video .inner").each(function(c,d){c=$(d).attr("data-poster");var e=$(d).attr("data-src");isNaN(c)?$(d).css("background-image","url("+c+")"):a(e,c).then(function(k){$(d).css("background-image","url("+k+")")});$(d).unbind("click").bind("click",function(){$("body").css("overflow","hidden");$(".j-video-preview").addClass("active");
$(".j-video-preview iframe").attr("src",window.JOE_CONFIG.THEME_URL+"/player.php?url="+e)})});$(".j-video-preview .close").unbind("click").bind("click",function(){$("body").css("overflow","");$(".j-video-preview").removeClass("active")})};b.prototype.init_wallpaper=function(){if(0!==$("#wallpaper-type").length){$.ajax({url:window.JOE_CONFIG.THEME_URL+"/wallpaperApi.php?cid=360tags",method:"get",dataType:"json",success:function(c){if("0"!==c.errno)return $.toast({type:"warning",message:"\u63a5\u53e3\u5f02\u5e38\uff0c\u8bf7\u8054\u7cfb\u5f00\u53d1\u8005\uff01"});
$(".j-wallpaper-load-1").hide();var d='<li data-cid="360new" class="active">\u6700\u65b0\u58c1\u7eb8</li>';c.data.forEach(function(e){d+='<li data-cid="'+e.id+'">'+e.name+"</li>"});$("#wallpaper-type").html(d);$("#wallpaper-type li").first().click()}});var a=this;$(document).on("click","#wallpaper-type li",function(){$(this).addClass("active").siblings().removeClass("active");a.wallpaper_page=0;a.wallpaper_cid=$(this).attr("data-cid");$("#wallpaper-list").html("");a.init_wallpaper_list()});$("#wallpaper-load").unbind("click").bind("click",
function(){a.wallpaper_page+=1;a.init_wallpaper_list()})}};b.prototype.init_wallpaper_list=function(){var a=this;$(".j-wallpaper-load-2").show();$.ajax({url:window.JOE_CONFIG.THEME_URL+"/wallpaperApi.php",data:{cid:a.wallpaper_cid,start:20*a.wallpaper_page,count:20},method:"get",dataType:"json",success:function(c){$(".j-wallpaper-load-2").hide();0!==c.total?(c.data.forEach(function(d){$("#wallpaper-list").append('\n                                <a class="item" data-fancybox="gallery" href="'+d.url+
'">\n                                    <img class="lazyload" src="'+window.JOE_CONFIG.DOCUMENT_LAZY_LOAD+'" data-src="'+d.img_1024_768+'" />\n                                </a>\n                            ')}),a.init_lazy_load()):$("#wallpaper-load").remove()}})};b.prototype.init_huya_type=function(){if(0!==$(".huya-list-type").length){var a=this;$(".huya-list-type .list ul li").unbind("click").bind("click",function(){window.location.href=a.changeURLArg(window.location.href,"vid",$(this).attr("data-vid"))})}};
b.prototype.init_huya_skip=function(){if(0!==$(".huya-list-go-play").length){var a=this;$(".huya-list-go-play").unbind("click").bind("click",function(){var c=a.changeURLArg(window.location.href,"play",$(this).attr("data-href"));c=a.changeURLArg(c,"title",$(this).attr("data-title"));window.open(c)})}};b.prototype.init_huya_pagination=function(){if(0!==$(".huya-list-pagination").length){var a=this;$(".huya-list-pagination li").on("click",function(){var c=window.location.href;c=a.changeURLArg(c,"pg",
$(this).attr("data-pg"));window.location.href=c})}};b.prototype.init_lazy_load=function(){jQuery('[data-fancybox="gallery"]').fancybox()};b.prototype.init_prefer_color_scheme=function(){function a(c){var d=document.cookie.replace(/(?:(?:^|.*;\s*)night\s*=\s*([^;]*).*$)|^.*$/,"$1");if(d){if("0"===d)return!1;if("1"===d)return!0}else return window.matchMedia&&window.matchMedia("(prefers-color-scheme: "+c+")").matches}!function(){var c;return $jscomp.asyncExecutePromiseGeneratorProgram(function(d){c=
document.querySelector(".js-promo-color-modes-toggle");var e;if(e=c)e=(e=document.querySelector("html[data-color-mode]"))?e.getAttribute("data-color-mode"):void 0,e="auto"===e;e&&((e=document.querySelector("html[data-color-mode]"))?(e=e.getAttribute("data-color-mode"),"auto"===e&&(e=a("dark")?"dark":a("light")?"light":void 0)):e=void 0,"dark"===e&&c.setAttribute("aria-checked","true"));d.jumpToEnd()})}();!function(){return $jscomp.asyncExecutePromiseGeneratorProgram(function(c){document.querySelector(".js-color-mode-settings")&&
window.history.replaceState({},document.title,document.URL.split("?")[0]);c.jumpToEnd()})}();this.global_item.color_mode_toggle_btn&&this.global_item.color_mode_toggle_btn.unbind("click").bind("click",function(c){c=c.currentTarget;var d=!0,e="dark";"true"===c.getAttribute("aria-checked")&&(d=!1,e="light");c.setAttribute("aria-checked",String(d));c=e;d=document.querySelector("html[data-color-mode]");document.cookie="dark"===c?"night=1;path=/":"night=0;path=/";d&&d.setAttribute("data-color-mode",c)})};
b.prototype.init_head_title=function(){if(document.querySelector("#post_top_title span")){var a=this.global_item.Jheader,c=$(".above"),d=$(".above .above-nav"),e=$(".below"),k=$("#post_top_title"),f=!0,m=!0,h=function(){k.addClass("post_no");d.removeClass("post_no");e.slideDown("fast",function(){f=!0})},g=function(){k.removeClass("post_no");d.addClass("post_no");e.slideUp("normal",function(){m=!0})};$(document).ready(function(){var l=0;if(768>screen.width)$(window).scroll(function(){var p=$(window).scrollTop();
395<p&&505>p||(450<=p?l>p&&m?(f=!1,c.slideDown("fast",function(){f=!0})):f&&(m=!1,c.slideUp("normal",function(){m=!0})):c.slideDown("fast",function(){f=!0}),l=p)});else{var n=a.offsetWidth;0<k.length&&750<n&&$(window).scroll(function(){var p=$(window).scrollTop();400<p&&500>p||(450<=p?l>p&&m?(f=!1,h()):f&&(m=!1,g()):h(),l=p)})}})}};b.prototype.init_navigation=function(){$("#siteNav li.nav-list-item").unbind("click").bind("click",function(a){a.preventDefault();$(this).siblings().removeClass("active");
$(this).addClass("active");window.scroll({top:$($(this).children("a").attr("href")).offset().top-($(".j-header").height()+20),behavior:"smooth"})})};b.prototype.init_mobinav=function(){$(".navigation-tab-item").bind("click",function(){$(".navigation-tab-item").removeClass("active");$(this).addClass("active");$(".navigation-tab-overlay").css({left:25*$(this).prevAll().length+"%"})});$("#mob_goTop").click(function(){$("#backToTop").click()});var a=$("#load_mobinav");a.hasClass("active")&&$(".navigation-tab-overlay").css({left:25*
a.prevAll().length+"%"})};b.prototype.init_jfloor=function(){function a(){k=[].slice.call(d.querySelectorAll("li"));k=k.map(function(g){var l=g.querySelector("a"),n=document.getElementById(l.getAttribute("data-href").slice(1));return{listItem:g,anchor:l,target:n}});k=k.filter(function(g){return!!g.target});var m=[],h;k.forEach(function(g,l){var n=g.anchor.offsetLeft-5,p=g.anchor.offsetTop,q=g.anchor.offsetHeight;0===l?(m.push("M",n,p,"L",n,p+q),g.pathStart=0):(h!==n&&m.push("L",h,p),m.push("L",n,
p),e.setAttribute("d",m.join(" ")),g.pathStart=e.getTotalLength()||0,m.push("L",n,p+q));h=n;e.setAttribute("d",m.join(" "));g.pathEnd=e.getTotalLength()});f=e.getTotalLength();c()}function c(){var m=window.innerHeight,h=f,g=0,l=0;k.forEach(function(n){var p=n.target.getBoundingClientRect();p.bottom>0*m&&p.top<1*m?(h=Math.min(n.pathStart,h),g=Math.max(n.pathEnd,g),l+=1,n.listItem.classList.add("visible")):n.listItem.classList.remove("visible")});0<l&&h<g?(e.setAttribute("stroke-dashoffset","1"),e.setAttribute("stroke-dasharray",
"1, "+h+", "+(g-h)+", "+f),e.setAttribute("opacity",1)):e.setAttribute("opacity",0)}if(0===$("#jFloor").length)return!1;var d=document.querySelector("#jFloor"),e=document.querySelector("#jFloor path"),k,f;window.addEventListener("resize",a,!1);window.addEventListener("scroll",c,!1);a()};b.prototype.init_resource_page=function(){function a(){$.ajax({url:window.location.href,method:"get",data:{category:c,tag:d,price:e,order:k},success:function(f){f=$(".row.posts-wrapper",f).html();$(".row.posts-wrapper").html(f)}})}
var c,d,e,k=null;$(".filter-tag.category li a").unbind("click").bind("click",function(f){f.preventDefault();c=$(this).data("mid");$(this).hasClass("on")&&(c=null);a();$(this).toggleClass("on");$(".term-bar .term-title").text($(this).text());$(this).parent().siblings("li").find("a").removeClass("on")});$(".filter-tag.tag li a").unbind("click").bind("click",function(f){f.preventDefault();d=$(this).data("mid");$(this).hasClass("on")&&(d=null);a();$(this).toggleClass("on");$(this).parent().siblings("li").find("a").removeClass("on")});
$(".filter-tag.price li a").unbind("click").bind("click",function(f){f.preventDefault();e=$(this).data("price");a();$(this).addClass("on");$(this).parent().siblings("li").find("a").removeClass("on")});$(".filter-tag.order li a").unbind("click").bind("click",function(f){f.preventDefault();k=$(this).data("order");a();$(this).addClass("on");$(this).parent().siblings("li").find("a").removeClass("on")})};"undefined"!==typeof module&&"undefined"!==typeof module.exports?module.exports=b:window.Joe=b})();
