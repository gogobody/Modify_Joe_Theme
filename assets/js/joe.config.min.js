...
cid:c.find(".cid").val()},success:function(e){var k=[],f="";k=$(e).contents();Array.from(k).forEach(function(h){"container"===h.parentNode.className&&(f=h)});if(/TypechoJoeTheme/.test(e)){var m=location.href;m=a.changeURLArg(m,"jscroll","comments");$.toast({type:"success",message:"\u5bc6\u7801\u6b63\u786e\uff0c\u5373\u5c06\u5237\u65b0\u672c\u9875\uff01"});setTimeout(function(){pjax.loadUrl(m)},a.options.reloadTime)}else $.toast({type:"warning",message:f.textContent||""})}})})};b.prototype.init_dynamic_verify=
...
q){f.set(p,k[q]||[])});e={};for(var m=$jscomp.makeIterator(f),h=m.next();!h.done;e={$jscomp$loop$prop$str$4:e.$jscomp$loop$prop$str$4},h=m.next()){var g=$jscomp.makeIterator(h.value);h=g.next().value;g=g.next().value.split("#");e.$jscomp$loop$prop$str$4="";g.forEach(function(p){return function(q){p.$jscomp$loop$prop$str$4+='\n\t\t\t\t\t\t\t\t<li data-link="'+d(q).link+'">'+d(q).name+"</li>\n\t\t\t\t\t\t\t"}}(e));$("#j-video-play").append('\n\t\t\t\t\t\t\t<div class="video-list-play">\n\t\t\t\t\t\t\t\t<div class="title">\u6e90\uff1a'+
...
[],m="";f=$(k).contents();Array.from(f).forEach(function(g){"container"===g.parentNode.className&&(m=g)});if(/TypechoJoeTheme/.test(k)){var h=location.href;h=a.changeURLArg(h,"jscroll","comments");$.toast({type:"success",message:"\u53d1\u9001\u6210\u529f\uff0c\u5373\u5c06\u5237\u65b0\u672c\u9875\uff01"});setTimeout(function(){pjax.loadUrl(h)},a.options.reloadTime)}else $.toast({type:"warning",message:m.textContent||""}),$(e).removeAttr("data-disabled")}}))})};b.prototype.init_video_album=function(){function a(c,
d,e,k){d=void 0===d?1:d;e=void 0===e?1:e;k=void 0===k?.5:k;var f=document.createElement("VIDEO");f.setAttribute("src",c);f.crossOrigin="*";f.currentTime=d;return new Promise(function(m,h){f.addEventListener("loadeddata",function(){var g=document.createElement("canvas");g.width=f.videoWidth*e;g.height=f.videoHeight*e;g.getContext("2d").drawImage(f,0,0,g.width,g.height);m(g.toDataURL("image/webp",k))})})}$(".j-short-video .inner").each(function(c,d){c=$(d).attr("data-poster");var e=$(d).attr("data-src");
...
(d=!1,e="light");c.setAttribute("aria-checked",String(d));c=e;d=document.querySelector("html[data-color-mode]");document.cookie="dark"===c?"night=1;path=/":"night=0;path=/";d&&d.setAttribute("data-color-mode",c)})};b.prototype.init_head_title=function(){if(document.querySelector("#post_top_title span")){var a=$("header.j-header"),c=$(".above"),d=$(".above .above-nav"),e=$(".below"),k=$("#post_top_title"),f=!0,m=!0,h=function(){k.addClass("post_no");d.removeClass("post_no");e.slideDown("fast",function(){f=
!0})},g=function(){k.removeClass("post_no");d.addClass("post_no");e.slideUp("normal",function(){m=!0})};$(document).ready(function(){var l=0;if(768>screen.width)$(window).scroll(function(){var p=$(window).scrollTop();395<p&&505>p||(450<=p?l>p&&m?(f=!1,c.slideDown("fast",function(){f=!0})):f&&(m=!1,c.slideUp("normal",function(){m=!0})):c.slideDown("fast",function(){f=!0}),l=p)});else{var n=a.width();0<k.length&&750<n&&$(window).scroll(function(){var p=$(window).scrollTop();400<p&&500>p||(450<=p?l>
p&&m?(f=!1,h()):f&&(m=!1,g()):h(),l=p)})}})}};b.prototype.init_navigation=function(){$("#siteNav li.nav-list-item").unbind("click").bind("click",function(a){a.preventDefault();$(this).siblings().removeClass("active");$(this).addClass("active");window.scroll({top:$($(this).children("a").attr("href")).offset().top-($(".j-header").height()+20),behavior:"smooth"})})};b.prototype.init_mobinav=function(){$(".navigation-tab-item").bind("click",function(){$(".navigation-tab-item").removeClass("active");$(this).addClass("active");
$(".navigation-tab-overlay").css({left:25*$(this).prevAll().length+"%"})});$("#mob_goTop").click(function(){$("#backToTop").click()});var a=$("#load_mobinav");a.hasClass("active")&&$(".navigation-tab-overlay").css({left:25*a.prevAll().length+"%"})};b.prototype.init_jfloor=function(){function a(){k=[].slice.call(d.querySelectorAll("li"));k=k.map(function(g){var l=g.querySelector("a"),n=document.getElementById(l.getAttribute("data-href").slice(1));return{listItem:g,anchor:l,target:n}});k=k.filter(function(g){return!!g.target});
var m=[],h;k.forEach(function(g,l){var n=g.anchor.offsetLeft-5,p=g.anchor.offsetTop,q=g.anchor.offsetHeight;0===l?(m.push("M",n,p,"L",n,p+q),g.pathStart=0):(h!==n&&m.push("L",h,p),m.push("L",n,p),e.setAttribute("d",m.join(" ")),g.pathStart=e.getTotalLength()||0,m.push("L",n,p+q));h=n;e.setAttribute("d",m.join(" "));g.pathEnd=e.getTotalLength()});f=e.getTotalLength();c()}function c(){var m=window.innerHeight,h=f,g=0,l=0;k.forEach(function(n){var p=n.target.getBoundingClientRect();p.bottom>0*m&&p.top<
1*m?(h=Math.min(n.pathStart,h),g=Math.max(n.pathEnd,g),l+=1,n.listItem.classList.add("visible")):n.listItem.classList.remove("visible")});0<l&&h<g?(e.setAttribute("stroke-dashoffset","1"),e.setAttribute("stroke-dasharray","1, "+h+", "+(g-h)+", "+f),e.setAttribute("opacity",1)):e.setAttribute("opacity",0)}if(0===$("#jFloor").length)return!1;var d=document.querySelector("#jFloor"),e=document.querySelector("#jFloor path"),k,f;window.addEventListener("resize",a,!1);window.addEventListener("scroll",c,
!1);a()};"undefined"!==typeof module&&"undefined"!==typeof module.exports?module.exports=b:window.Joe=b})();window.JoeInstance=new Joe({});
