...
message:"\u8bf7\u8f93\u5165\u6b63\u786e\u7684\u90ae\u7bb1\uff01"});b=$("#comment-content");if("canvas"===$("#commentTypeContent .canvas").attr("data-type")){var e=$("#draw")[0].toDataURL("image/webp",.1);b.val("{!{"+e+"}!} ")}if(""===b.val().trim())return $.toast({type:"warning",message:"\u8bf7\u8f93\u5165\u8bc4\u8bba\u5185\u5bb9\uff01"});$(this).attr("data-disabled")||($(this).attr("data-disabled",!0),$.ajax({url:$(this).attr("action"),type:"post",data:$(this).serializeArray(),success:function(k){var f=
[],m="";f=$(k).contents();Array.from(f).forEach(function(g){"container"===g.parentNode.className&&(m=g)});if(/TypechoJoeTheme/.test(k)){var h=location.href;h=a.changeURLArg(h,"jscroll","comments");$.toast({type:"success",message:"\u53d1\u9001\u6210\u529f\uff0c\u5373\u5c06\u5237\u65b0\u672c\u9875\uff01"});setTimeout(function(){window.location.href=h},a.options.reloadTime)}else $.toast({type:"warning",message:m.textContent||""}),$(c).removeAttr("data-disabled")}}))})};d.prototype.init_wap_cloud=function(){var a=
function(b,c){return Math.floor(Math.random()*(c-b+1))+b};$("#search-cloud a").each(function(b,c){$(c).css("background","rgba("+a(0,255)+", "+a(0,255)+", "+a(0,255)+", "+a(.8,1)+")")})};d.prototype.init_wap_search=function(){$(".search-toggle-xs").on("click",function(){$(".j-slide").removeClass("active");$(".j-sidebar-xs").removeClass("active");var a=$(".j-search-down-xs");a.toggleClass("active");a.hasClass("active")?($("body").css("overflow","hidden"),$(".j-header").css("box-shadow","none")):($("body").css("overflow",
""),$(".j-header").css("box-shadow",""))});$(".j-search-down-xs .mask").on("click",function(){$(".j-search-down-xs").removeClass("active");$("body").css("overflow","");$(".j-header").css("box-shadow","")})};d.prototype.init_wap_sidebar=function(){$(".j-slide").on("click",function(a){$(".j-search-down-xs").removeClass("active");$("body").css("overflow","");$(".j-header").css("box-shadow","");$(this).toggleClass("active");$(".j-sidebar-xs").toggleClass("active");$(this).hasClass("active")?$("body").css("overflow",
...
method:"get",data:{ac:"list",at:"json"},dataType:"json",success:function(b){if(1!==b.code)return $.toast({type:"warning",message:"\u83b7\u53d6\u5217\u8868\u5931\u8d25\uff01"});$(".j-video-load-1").hide();var c='<li class="active">\u5168\u90e8</li>',e=window.JOE_CONFIG.VIDEO_LIST_SHIELD.split("||").map(function(k){return k.trim()});b.class.forEach(function(k){e.some(function(f){return f===k.type_name})||(c+='<li data-id="'+k.type_id+'">'+k.type_name+"</li>")});$("#j-video-type").html(c)}});var a=this;
...
0!=k.vod_year?"<i>"+k.vod_year+"</i>":"")+"\n                                    </a>\n                                </li>\n                            ")}),c.init_lazy_load(),c.video_canLoad=!0)}}))}};d.prototype.init_video_search=function(){var a=this;$("#j-video-search button").on("click",function(){if(""===$("#j-video-search input").val().trim())return $.toast({type:"info",message:"\u8bf7\u8f93\u5165\u5185\u5bb9\uff01"});a.video_page=0;$("#j-video-list").html("");a.init_video_list(null,$("#j-video-search input").val())})};
d.prototype.init_load_more_video=function(){if(0!==$("#j-video-list").length){var a=this;$(window).on("scroll",function(){var b=$(window).scrollTop(),c=$(window).height(),e=$("#j-video-list").offset().top+$("#j-video-list").height();b+c>=e&&a.init_video_list()})}};d.prototype.init_video_detail=function(){var a=this,b=(new URLSearchParams(window.location.search)).get("vod_id");b&&($(".j-video-load-3").show(),$.ajax({url:window.JOE_CONFIG.VIDEO_LIST_API,method:"get",data:{ac:"detail",at:"json",ids:b},
dataType:"json",success:function(c){function e(q){q=q.split("$");return{name:q[0]||"",link:q[1]||""}}if(1!==c.code||1!==c.list.length)return $.toast({type:"warning",message:"\u6570\u636e\u83b7\u53d6\u5931\u8d25\uff01"});$(".j-video-load-3").hide();c=c.list[0];$("#j-video-info").html('\n\t\t\t\t\t\t<div class="image">\n\t\t\t\t\t\t\t<img class="lazyload" src="'+window.JOE_CONFIG.DOCUMENT_LAZY_LOAD+'" alt="'+c.vod_name+'" data-original="'+c.vod_pic+'" title="'+c.vod_name+'">\n\t\t\t\t\t\t\t'+(c.vod_year&&
0!==c.vod_year?"<i>"+c.vod_year+"</i>":"")+"\n\t\t\t\t\t\t</div>\n\t\t\t\t\t\t<dl>\n\t\t\t\t\t\t\t<dt>"+(c.vod_name+(c.vod_remarks?" - "+c.vod_remarks:""))+"</dt>\n\t\t\t\t\t\t\t<dd>\n\t\t\t\t\t\t\t\t<span>\u7c7b\u578b\uff1a</span>\n\t\t\t\t\t\t\t\t<span>"+(c.vod_class||"\u672a\u77e5")+"</span>\n\t\t\t\t\t\t\t</dd>\n\t\t\t\t\t\t\t<dd>\n\t\t\t\t\t\t\t\t<span>\u4e3b\u6f14\uff1a</span>\n\t\t\t\t\t\t\t\t<span>"+(c.vod_actor||"\u672a\u77e5")+"</span>\n\t\t\t\t\t\t\t</dd>\n\t\t\t\t\t\t\t<dd>\n\t\t\t\t\t\t\t\t<span>\u5bfc\u6f14\uff1a</span>\n\t\t\t\t\t\t\t\t<span>"+
...
"/player.php?url="+e)})});$(".j-video-preview .close").on("click",function(){$("body").css("overflow","");$(".j-video-preview").removeClass("active")})};d.prototype.init_lazy_load=function(){new LazyLoad(".lazyload")};d.prototype.init_head_title=function(){var a=$("header.j-header"),b=$(".row.above .above-nav"),c=$(".row.below"),e=$("#post_top_title"),k=!0,f=!0,m=function(){e.addClass("post_no");b.removeClass("post_no");c.slideDown("fast",function(){k=!0})},h=function(){e.removeClass("post_no");b.addClass("post_no");
c.slideUp("normal",function(){f=!0})};$(document).ready(function(){if(!(768>screen.width)){var g=a.width(),l=0;0<e.length&&750<g&&$(window).scroll(function(){var n=$(window).scrollTop();400<n&&500>n||(450<=n?l>n&&f?(k=!1,m()):k&&(f=!1,h()):m(),l=n)})}})};d.prototype.init_navigation=function(){$("#siteNav li.nav-list-item").on("click",function(a){a.preventDefault();$(this).siblings().removeClass("active");$(this).addClass("active");window.scroll({top:$($(this).children("a").attr("href")).offset().top-
($(".j-header").height()+20),behavior:"smooth"})})};d.prototype.init_mobinav=function(){$(".navigation-tab-item").bind("click",function(){$(".navigation-tab-item").removeClass("active");$(this).addClass("active");$(".navigation-tab-overlay").css({left:25*$(this).prevAll().length+"%"})});$("#mob_goTop").click(function(){$("#backToTop").click()});var a=$("#load_mobinav");a.hasClass("active")&&$(".navigation-tab-overlay").css({left:25*a.prevAll().length+"%"})};"undefined"!==typeof module&&"undefined"!==
typeof module.exports?module.exports=d:window.Joe=d})();window.JoeInstance=new Joe({});
