                ...
                function fileUploadComplete(id, url, data) {
                    var li = $('#' + id).removeClass('loading').data('cid', data.cid)
                        ...
                        .html('<input type="hidden" name="attachment[]" value="' + data.cid + '" />' +
                            '<a class="insert" target="_blank" rel="noopener" href="###" title="<?php _e('点击插入文件'); ?>">' + data.title + '</a><div class="info">' + data.bytes +
                            ' <a class="file" target="_blank" rel="noopener" href="<?php $options->adminUrl('media.php'); ?>?cid=' +
 ...
 */
global $cachedir;
$cachedir = Helper::options()->themeFile('Typecho-Joe-Theme','txtcache/');
function exicache($fis)
{
    $i = false;
    global $cachedir;
    if ($fis == 'pagehot') {
        if (file_exists($cachedir."pagehot.txt") && (!empty(file_get_contents($cachedir."pagehot.txt")))) {
            $i = true;
        }
    }
    if ($fis == 'sequhot') {
        if (file_exists($cachedir."sequhot.txt") && (!empty(file_get_contents($cachedir."sequhot.txt")))) {
            $i = true;
        }
    }
    if ($fis == 'hot') {
        if (file_exists($cachedir."hot.txt") && (!empty(file_get_contents($cachedir."hot.txt")))) {
            $i = true;
        }
    }
    if ($fis == 'pagemember') {
        if (file_exists($cachedir."pagemember.txt") && (!empty(file_get_contents($cachedir."pagemember.txt")))) {
            $i = true;
        }
    }
    if ($fis == 'siderping') {
        if (file_exists($cachedir."siderping.txt") && (!empty(file_get_contents($cachedir."siderping.txt")))) {
            $i = true;
        }
    }
    if ($fis == 'likehot') {
        if (file_exists($cachedir."likehot.txt") && (!empty(file_get_contents($cachedir."likehot.txt")))) {
            $i = true;
        }
    }
...
function fosmember(){
    $html='';
    global $cachedir;
    $fileshot = $cachedir."pagemember.txt";
